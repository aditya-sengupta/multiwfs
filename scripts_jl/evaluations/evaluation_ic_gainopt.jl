...
function controllers_from_params(gain_slow, gain_fast, leak=0.999)
	# controller setup
	fast_controller = FilteredIntegrator(gain_fast, leak, ZPKFilter(0, 0, 1), 1/f_loop)
...
begin
	fast_controller, slow_controller = controllers_from_params(gain_slow, gain_fast)
...
begin
	...
	parameter_centers = [gain_slow, gain_fast]
	parameter_grids = [(gain_slow-0.1):0.01:(gain_slow+0.1), (gain_fast-0.1):0.01:(gain_fast+0.1)]
	# resolved parameter minimum grid
	min_plots = []
	for (i, (parname, pargrid)) in enumerate(zip(parameter_names, parameter_grids))
		Xerrs_parsweep = []
		for par in pargrid
			fast_controller, slow_controller = controllers_from_params([i == j ? par : parameter_centers[j] for j in 1:2]...)
...
begin
	...
	pf = plot(allplots..., size=(1100,800), left_margin=5mm, suptitle="Gain-optimized IC; gain_slow=$gain_slow, gain_fast=$gain_fast, r0 NCP = $(r0_ncp)m", dpi=300)
	Plots.savefig(joinpath(multiwfs.PROJECT_ROOT, "figures", "evaluation", "evaluation_gainopt_r0ncp$(r0_ncp).pdf"))
	pf
end
