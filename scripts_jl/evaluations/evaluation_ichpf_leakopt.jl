...
function controllers_from_params(gain_slow, gain_fast, f_cutoff, leak_slow, leak_fast)
	# controller setup
...
begin
	fast_controller, slow_controller = controllers_from_params(gain_slow, gain_fast, f_cutoff, leak_slow, leak_fast)
...
begin
	parameter_names = ["gain_slow", "gain_fast", "f_cutoff", "leak_slow", "leak_fast"]
	parameter_centers = [gain_slow, gain_fast, f_cutoff, leak_slow, leak_fast]
	parameter_grids = [(gain_slow-0.1):0.01:(gain_slow+0.1), (gain_fast-0.1):0.01:(gain_fast+0.1), (f_cutoff-10):1:(f_cutoff+10), nines_range(leak_slow), nines_range(leak_fast)]
...
begin
	...
	pf = plot(allplots..., size=(1100,1100), left_margin=5mm, suptitle="Leak-optimized IC-HPF; gain_slow=$gain_slow, gain_fast=$gain_fast, f_cutoff=$f_cutoff \n leak_slow=$leak_slow, leak_fast=$leak_fast, r0 NCP = $(r0_ncp)m", dpi=300, layout=(4, 3))
	Plots.savefig(joinpath(multiwfs.PROJECT_ROOT, "figures", "evaluation", "evaluation_ichpf_leakopt_r0ncp$(r0_ncp).png"))
	pf
end
