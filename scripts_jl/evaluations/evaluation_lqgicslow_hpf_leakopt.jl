...
begin
	fast_controller, slow_controller = controllers_from_params(log_lqg_noise, gain_fast, f_cutoff, leak_slow, leak_fast)
...
function nines_range(leak)
	...
	return max.(0, @. 1 - exp10(-r))
end;

# ╔═╡ 2d3b6440-3c01-483f-b9c4-b2f8a867817e
begin
	parameter_names = ["log_lqg_noise", "gain_fast", "f_cutoff", "leak_slow", "leak_fast"]
	parameter_centers = [log_lqg_noise, gain_fast, f_cutoff, leak_slow, leak_fast]
	parameter_grids = [(log_lqg_noise-1):0.1:(log_lqg_noise+1), (gain_fast-0.1):0.01:(gain_fast+0.1), (f_cutoff-10):1:(f_cutoff+10), nines_range(leak_slow), nines_range(leak_fast)]
